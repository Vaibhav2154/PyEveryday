...
const scriptCategories = [
  {
    ...
    scripts: ["Auto Email Sender", "File Organizer", "Backup Scheduler", "File Renamer", "Folder Monitor"],
    color: "from-cyan-400 to-blue-600",
    hoverColor: "hover:from-cyan-500 hover:to-blue-700",
    bgPattern: "linear-gradient(45deg, rgba(6,182,212,0.1) 25%, transparent 25%), linear-gradient(-45deg, rgba(6,182,212,0.1) 25%, transparent 25%)",
    icon: "⚡"
  },
  {
    emoji: "🧠",
    ...
    scripts: ["Pomodoro Timer", "Quote Fetcher", "Reminder System", "Time Tracker", "Todo Manager"],
  ...
  },
  {
    ...
    scripts: ["News Fetcher", "Weather Checker", "Web Scraper", "YouTube Downloader"],
    color: "from-green-400 to-teal-600",
    hoverColor: "hover:from-green-500 hover:to-teal-700",
    bgPattern: "repeating-linear-gradient(90deg, transparent, transparent 2px, rgba(34,197,94,0.1) 2px, rgba(34,197,94,0.1) 4px)",
    icon: "🌊"
  },
  {
    emoji: "⚙️",
    ...
    scripts: ["Age Calculator", "Currency Converter", "Password Generator", "Unit Converter"],
    color: "from-orange-400 to-red-600",
    hoverColor: "hover:from-orange-500 hover:to-red-700",
    bgPattern: "conic-gradient(from 0deg, rgba(251,146,60,0.1), rgba(239,68,68,0.1), rgba(251,146,60,0.1))",
    icon: "🔧"
  },
  {
    emoji: "📊",
    ...
    bgPattern: "linear-gradient(135deg, rgba(99,102,241,0.1) 0%, rgba(168,85,247,0.1) 100%)",
    icon: "📈"
  },
  {
    emoji: "🔐",
    ...
    bgPattern: "radial-gradient(ellipse at center, rgba(239,68,68,0.1) 0%, transparent 70%)",
    icon: "🛡️"
  },
  {
    emoji: "🎵",
    ...
    bgPattern: "linear-gradient(60deg, rgba(251,191,36,0.1) 25%, transparent 25%), linear-gradient(120deg, rgba(251,191,36,0.1) 25%, transparent 25%)",
    icon: "🎨"
  }
];
...
export default function Home() {
  ...
  const handleTerminalSubmit = (e: React.FormEvent) => {
    ...
    if (terminalInput.trim()) {
      setTerminalHistory(prev => [...prev, `$ ${terminalInput}`, "Command executed successfully! ✅"]);
      setTerminalInput("");
    }
  };
    ...
    <div className="min-h-screen bg-black text-white overflow-hidden relative">
      ...
      <div 
        ...
        style={{
          ...
          transform: `scale(${isLoaded ? 1 : 0}) ${hoveredCategory !== null ? 'scale(1.5)' : 'scale(1)'}`,
      ...
      <section ref={heroRef} className="h-screen flex items-center justify-center relative">
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute top-1/2 right-1/3 w-24 h-24 bg-green-400/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }}></div>
        </div>

        <div className="max-w-6xl mx-auto px-4 text-center relative z-10">
          {/* Enhanced Terminal Window */}
          <div className={`bg-gray-900/90 backdrop-blur-lg rounded-lg border border-gray-700 shadow-2xl transition-all duration-1000 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'} hover:shadow-cyan-500/25 hover:shadow-2xl`}>
            {/* Terminal Header with interactive buttons */}
            <div className="flex items-center justify-between p-4 border-b border-gray-700">
              <div className="flex space-x-2">
                <button className="w-3 h-3 bg-red-500 rounded-full hover:bg-red-400 transition-colors" onClick={() => setTerminalHistory([])}></button>
                <button className="w-3 h-3 bg-yellow-500 rounded-full hover:bg-yellow-400 transition-colors"></button>
                <button className="w-3 h-3 bg-green-500 rounded-full hover:bg-green-400 transition-colors" onClick={() => setIsTerminalFocused(!isTerminalFocused)}></button>
              </div>
              <span className="text-gray-400 font-mono text-sm">
                user@pyeveryday:~$ {scriptCategories[activeScript].name.toLowerCase()}
              </span>
            ...
            <div className="p-8 font-mono text-left max-h-96 overflow-y-auto">
              <div className="text-green-400 mb-2">$ python3 pyeveryday.py --initialize</div>
              ...
              <div className="text-yellow-400 mb-4 bg-yellow-400/10 p-2 rounded border-l-4 border-yellow-400">
                <span className="flex items-center">
                  <span className="mr-2">{scriptCategories[activeScript].icon}</span>
                  Active Module: <span className="text-cyan-400 ml-1 font-bold">{scriptCategories[activeScript].name}</span>
                </span>
              </div>
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                {scriptCategories[activeScript].scripts.map((script, index) => (
                  <span 
                    ...
                    onClick={() => setTerminalHistory(prev => [...prev, `$ Running ${script}...`, `✅ ${script} executed successfully!`])}
                  >
                    {script}
                  </span>
                ))}
              ...
              <form onSubmit={handleTerminalSubmit} className="flex items-center">
                ...
                />
                <span className={`text-green-400 ${showCursor ? 'opacity-100' : 'opacity-0'} transition-opacity`}>_</span>
              </form>
            </div>
          </div>
          ...
          <div className="relative max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {scriptCategories.map((category, index) => (
                <div 
                  key={index}
                  className={`group relative overflow-hidden rounded-2xl bg-gradient-to-br ${category.color} p-6 hover:scale-110 transition-all duration-500 cursor-pointer ${category.hoverColor} ${selectedCategory === index ? 'ring-4 ring-cyan-400 scale-105' : ''}`}
                  style={{
                    clipPath: index % 3 === 0 ? 'polygon(20% 0%, 80% 0%, 100% 20%, 100% 80%, 80% 100%, 20% 100%, 0% 80%, 0% 20%)' : 'none',
                  ...
                  <div className="relative z-10">
                    ...
                    <div className="space-y-2">
                      {category.scripts.slice(0, selectedCategory === index ? category.scripts.length : 3).map((script, scriptIndex) => (
                        <div 
                          key={scriptIndex} 
                          className={`text-xs text-white/80 bg-white/20 rounded px-2 py-1 backdrop-blur-sm transition-all duration-300 hover:bg-white/30 cursor-pointer transform ${selectedCategory === index ? 'translate-x-0 opacity-100' : scriptIndex >= 3 ? 'translate-x-4 opacity-0' : 'translate-x-0 opacity-100'}`}
                          style={{ transitionDelay: `${scriptIndex * 50}ms` }}
                          onClick={(e) => {
                            e.stopPropagation();
                            setTerminalHistory(prev => [...prev, `$ python ${category.name.toLowerCase()}/${script.toLowerCase().replace(/\s+/g, '_')}.py`, `✅ ${script} executed successfully!`]);
                          }}
                        >
                          {script}
                        </div>
                    ...
                    <div className="absolute top-2 right-2 flex space-x-1">
                      {[...Array(3)].map((_, i) => (
                        <div 
                          key={i}
                          className={`w-2 h-2 rounded-full transition-all duration-300 ${hoveredCategory === index ? 'bg-white' : 'bg-white/30'}`}
      ...
      <section className="py-20 relative">
        <div className="max-w-6xl mx-auto px-4">
          ...
          <div className={`bg-gray-800/95 backdrop-blur-lg rounded-lg overflow-hidden shadow-2xl border border-gray-700 transition-all duration-500 ${showCodeDemo ? 'transform scale-105 shadow-cyan-500/25' : ''}`}>
            <div className="flex items-center justify-between p-4 bg-gray-900 border-b border-gray-700">
              ...
              <div className="flex items-center space-x-4">
                <span className="text-gray-400 font-mono text-sm">file_organizer.py</span>
                <div className="flex space-x-2 text-xs">
                  <span className="px-2 py-1 bg-blue-500/20 text-blue-400 rounded">Python</span>
            ...
            <div className="p-6 font-mono text-sm relative">
              ...
              <div className="ml-8">
                ...
                <div className="text-purple-400">
                  import <span className="text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer">os</span>, <span className="text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer">shutil</span>
                </div>
                <div className="text-purple-400">
                  from <span className="text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer">pathlib</span> import <span className="text-cyan-400 hover:text-cyan-300 transition-colors cursor-pointer">Path</span>
                </div>
                <br />
                <div className="text-purple-400">
                  def <span className="text-yellow-400 hover:text-yellow-300 transition-colors cursor-pointer">organize_files</span>(<span className="text-cyan-400">directory</span>):
                </div>
                <div className="ml-4 text-gray-300">
                  ...
                  <div>
                    <span className="text-purple-400">for</span> file <span className="text-purple-400">in</span> Path(directory).iterdir():
                  </div>
                  <div className="ml-4">
                    <div>
                      <span className="text-purple-400">if</span> file.is_file():
                    </div>
                    <div className="ml-4">
              ...
              {showCodeDemo && (
                <div className="absolute bottom-4 right-4">
                  <button 
                    ...
                    onClick={() => setTerminalHistory(prev => [...prev, '$ python file_organizer.py --execute', '🗂️  Organizing files...', '✅ 47 files organized successfully!', '📊 Created folders: Documents, Images, Videos, Others'])}
                  >
                    Run Code
                  </button>
                </div>
      ...
      <section className="py-20 relative">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className={`bg-gradient-to-br from-cyan-900/50 to-blue-900/50 rounded-2xl p-8 border border-cyan-500/30 backdrop-blur-sm hover:scale-105 transition-all duration-500 cursor-pointer group ${statsAnimated ? 'animate-bounce' : ''}`}>
              ...
              <div className="text-xl font-semibold text-white mb-2">Python Scripts</div>
            ...
            <div className={`bg-gradient-to-br from-purple-900/50 to-pink-900/50 rounded-2xl p-8 border border-purple-500/30 backdrop-blur-sm hover:scale-105 transition-all duration-500 cursor-pointer group ${statsAnimated ? 'animate-bounce' : ''}`} style={{ animationDelay: '0.2s' }}>
              <div className="relative">
                <div className="text-6xl font-bold text-purple-400 mb-4 group-hover:scale-110 transition-transform duration-300">
                  {statsAnimated ? '7' : '0'}
                </div>
                <div className="absolute -top-2 -right-2 w-6 h-6 bg-purple-400/20 rounded-full animate-ping" style={{ animationDelay: '0.5s' }}></div>
              </div>
              <div className="text-xl font-semibold text-white mb-2">Categories</div>
              <div className="text-gray-400">Covering all aspects of automation</div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                {scriptCategories.slice(0, 4).map((cat, i) => (
                  <div key={i} className="bg-purple-400/10 rounded px-2 py-1 text-xs text-purple-300">
                    {cat.emoji} {cat.name}
                  </div>
                ))}
              </div>
            ...
            <div className={`bg-gradient-to-br from-green-900/50 to-teal-900/50 rounded-2xl p-8 border border-green-500/30 backdrop-blur-sm hover:scale-105 transition-all duration-500 cursor-pointer group ${statsAnimated ? 'animate-bounce' : ''}`} style={{ animationDelay: '0.4s' }}>
              <div className="relative">
                <div className="text-6xl font-bold text-green-400 mb-4 group-hover:scale-110 transition-transform duration-300">
                  ∞
                </div>
                <div className="absolute -top-2 -right-2 w-6 h-6 bg-green-400/20 rounded-full animate-ping" style={{ animationDelay: '1s' }}></div>
              </div>
              <div className="text-xl font-semibold text-white mb-2">Possibilities</div>
              <div className="text-gray-400">Unlimited automation potential</div>
              <div className="mt-4 space-y-2">
                ...
                <div className="flex items-center text-sm">
                  <div className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse" style={{ animationDelay: '0.3s' }}></div>
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
                <div className="bg-gray-800/50 rounded-lg p-4 hover:bg-gray-700/50 transition-all duration-300 cursor-pointer group/feature">
                  <div className="text-2xl mb-2 group-hover/feature:scale-125 transition-transform duration-300">⚡</div>
                  <div className="text-sm text-gray-300">Instant Setup</div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4 hover:bg-gray-700/50 transition-all duration-300 cursor-pointer group/feature">
                  <div className="text-2xl mb-2 group-hover/feature:scale-125 transition-transform duration-300">🔧</div>
                  <div className="text-sm text-gray-300">Easy Customization</div>
                </div>
                <div className="bg-gray-800/50 rounded-lg p-4 hover:bg-gray-700/50 transition-all duration-300 cursor-pointer group/feature">
                  <div className="text-2xl mb-2 group-hover/feature:scale-125 transition-transform duration-300">🚀</div>
      ...
      <footer className="py-16 border-t border-gray-800 relative overflow-hidden">
        ...
        <div className="relative max-w-6xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-400 mb-8 text-lg hover:text-gray-300 transition-colors duration-300 cursor-default">
          ...
          <div className="flex justify-center space-x-8 mb-8">
            <a 
              ...
              target="_blank" 
              rel="noopener noreferrer" 
              className="text-gray-400 hover:text-cyan-400 transition-all duration-300 font-medium hover:scale-105 transform relative group"
            >
            ...
            <button className="text-gray-400 hover:text-cyan-400 transition-all duration-300 font-medium hover:scale-105 transform relative group">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center space-y-4 md:space-y-0">
              ...
              <div className="flex space-x-4">
                ...
                <div className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center hover:bg-purple-500 transition-all duration-300 cursor-pointer group">
                  <span className="text-gray-400 group-hover:text-white text-xs font-bold">𝕏</span>
                </div>
                <div className="w-8 h-8 bg-gray-800 rounded-full flex items-center justify-center hover:bg-blue-500 transition-all duration-300 cursor-pointer group">
                  <span className="text-gray-400 group-hover:text-white text-xs font-bold">in</span>
                </div>
              </div>
            </div>
          </div>
